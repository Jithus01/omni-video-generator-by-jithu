...
const Button: React.FC<ButtonProps> = ({ children, onClick, variant = 'primary', isActive = false, className, ...props }) => {
  let styles = 'transition-all duration-150 rounded-md font-medium text-sm disabled:opacity-50';
  ...
  const variantClasses: Record<string, string> = {
    ...
    action: 'border border-gray-600 text-white hover:bg-[#FF6633] hover:text-black flex items-center justify-center py-2.5 px-3 space-x-2',
    tag: isActive 
...
const ControlGroup: React.FC<ControlGroupProps> = ({ title, children, isGrid = false }) => (
  <div className="mb-8">
    <h3 className="text-gray-400 font-semibold uppercase text-xs tracking-wider mb-3">{title}</h3>
    <div className={`${isGrid ? 'grid grid-cols-3 gap-2' : 'flex flex-wrap gap-2'}`}>{children}</div>
  </div>
);
...
const AngleButton: React.FC<AngleButtonProps> = ({ label, icon, isActive, onClick }) => {
  const activeClass = isActive ? 'border-[#FF6633] bg-[#2d1b11] text-white' : 'border-gray-700 bg-transparent text-gray-500 hover:border-gray-500';
  return (
    <button
      onClick={onClick}
...
const SliderControl: React.FC<{ label: string; min: number; max: number; value: number; setValue: (v: number) => void; unit: string; }> = ({ label, min, max, value, setValue, unit }) => (
  <div className="mb-8">
    <div className="flex justify-between items-baseline mb-2">
      <h3 className="text-gray-400 font-semibold uppercase text-xs tracking-wider">{label}</h3>
      <span className="text-white text-sm">{value}{unit}</span>
    </div>
    <input
      type="range"
      min={min}
...
export default function Page() {
  ...
  const handleStyleToggle = (tag: string) => {
    setActiveStyles(prev => prev.includes(tag) ? prev.filter(t => t !== tag) : [...prev, tag]);
  };
  ...
  return (
    <div className="min-h-screen bg-[#111111] text-white p-4 md:p-8 font-inter">
      ...
      <header className="flex justify-between items-center mb-6 border-b border-gray-800 pb-4">
        <div className="text-xl font-bold tracking-wider">OMNI VIDEO STUDIO</div>
        <div className="text-2xl font-bold tracking-wider text-gray-500">VIDEO GENERATOR</div>
        <HomeIcon
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
        ...
        <div className="p-4 bg-[#222222] rounded-xl md:col-span-1">
          ...
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-gray-400 font-semibold uppercase text-xs tracking-wider">Resolution</h3>
            <span className="text-white">1080p</span>
          </div>
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-gray-400 font-semibold uppercase text-xs tracking-wider">Frame Rate</h3>
            <span className="text-white">30 fps</span>
          </div>

          {/* Audios Toggle */}
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-gray-400 font-semibold uppercase text-xs tracking-wider">Audios</h3>
            <div className="relative inline-block w-10 mr-2 align-middle select-none transition duration-200 ease-in">
              ...
              />
              <label className={`block h-6 rounded-full cursor-pointer transition-all duration-300 ease-in-out ${audiosEnabled ? 'bg-[#FF6633]' : 'bg-gray-700'}`}>
                <span className={`absolute left-0 top-0 block w-6 h-6 rounded-full border-4 transition-transform duration-300 ease-in-out ${audiosEnabled ? 'bg-white border-[#FF6633] translate-x-4' : 'bg-white border-gray-700'}`}></span>
              </label>
            </div>
          </div>
        </div>
        ...
        <div className="col-span-1 md:col-span-3 flex flex-col space-y-4">
          <div className="flex space-x-2 bg-[#222222] p-2 rounded-xl">
            ...
            />
            <Button variant="primary" className="py-2.5 px-6">GENERATE</Button>
          </div>
          ...
          {/* History */}
          <div className="mt-4">
            <h3 className="text-gray-300 font-semibold text-lg mb-3">History</h3>
            <div className="flex space-x-3 overflow-x-auto pb-2">
              {[1, 2, 3, 4, 5].map(i => (
                <div key={i} className="w-24 h-16 rounded-lg overflow-hidden flex-shrink-0 border-2 border-transparent hover:border-[#FF6633] cursor-pointer">
                  <img src={`https://placehold.co/100x70/222/FFF?text=Clip+${i}`} alt={`History clip ${i}`} className="w-full h-full object-cover" />
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Right Column */}
        <div className="p-4 bg-[#222222] rounded-xl md:col-span-1">
          <h3 className="text-gray-300 font-semibold text-lg mb-4">Output & Assets</h3>

          <div className="mb-6">
            <div className="w-full h-32 bg-gray-800 rounded-lg overflow-hidden flex items-center justify-center">
              <img src="https://placehold.co/150x100/333/FFF?text=Output+Preview" alt="Output Asset" className="w-full h-full object-cover" style={{ filter: 'brightness(0.9)' }} />
            </div>
            ...
            onClick={() => {
              ...
              input.onchange = (e: any) => {
                ...
                if (files && files.length > 0) {
                    alert(`${files.length} file(s) selected: ${Array.from(files).map(f => (f as File).name).join(", ")}`);
                }
          ...
          {/* Export */}
          <div className="mb-6">
            <h4 className="text-gray-400 font-semibold uppercase text-xs tracking-wider mb-2">Export</h4>
          ...
          <div className="space-y-3">
            <Button variant="action" className="w-full" onClick={() => {
            ...
            <Button variant="action" className="w-full">SHARE</Button>
            <Button variant="action" className="w-full">REGENERATE</Button>
            <Button variant="action" className="w-full">SAVE</Button>
          </div>
        </div>
